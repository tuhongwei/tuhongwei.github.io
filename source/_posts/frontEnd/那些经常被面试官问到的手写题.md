...
function isThenable (obj) {
    return (isFunction(obj) || isObject(obj)) && typeof obj.then === 'function';

function transition (promise, state, result) {
    if (promise.state !== 'pending') return;
    promise.state = state;
    promise.result = result;
    promise.callbacks.forEach(callback => setTimeout(() => handleCallback(callback, state, result)));
}
function resolvePromise (promise, result, resolve, reject) {
    if (promise === result) {
        ...
        new Promise(result.then).then(resolve, reject);
    } else {
        resolve(result);
    }
}
